...
func TestDLQHeaders_SanitizedOperationalMetadata(t *testing.T) {
	meta := DLQFailureMeta{
		Consumer:        DefaultGatewayKafkaGroup,
		SourceTopic:     DefaultSessionInvalidatedTopic,
		SourcePartition: 2,
		SourceOffset:    9,
		AttemptCount:    4,
	...
	want := map[string]string{
		dlqHeaderConsumer:        DefaultGatewayKafkaGroup,
		dlqHeaderSourceTopic:     DefaultSessionInvalidatedTopic,
...
func TestFranzSessionInvalidatedClient_ConfigRequiresIdentity(t *testing.T) {
	_, err := newFranzSessionInvalidatedClient(SessionInvalidatedKafkaConfig{})
	if err == nil {
