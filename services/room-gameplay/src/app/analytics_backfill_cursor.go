...
const (
	analyticsBackfillCursorPrefix = "ab1."
	// documentedNonProdAnalyticsBackfillCursorMACKey is capability/offline fallback only.
	// Durable API mode requires ROOM_ANALYTICS_BACKFILL_CURSOR_SECRET before ready/wiring.
	documentedNonProdAnalyticsBackfillCursorMACKey = "unoarena-room-analytics-backfill-cursor-v1-nonprod"
)
...
var ErrAnalyticsBackfillCursorSecretRequired = errors.New("ROOM_ANALYTICS_BACKFILL_CURSOR_SECRET required")
...
func AnalyticsBackfillCursorSecretConfigured() bool {
	if strings.TrimSpace(os.Getenv("ROOM_ANALYTICS_BACKFILL_CURSOR_SECRET")) != "" {
		return true
	}
...
func DecodeAnalyticsBackfillCursor(raw string) (AnalyticsBackfillCursor, error) {
	...
	if strings.Contains(raw, "OFFSET") || strings.Contains(raw, "offset=") ||
		strings.Contains(raw, "outbox_id") || strings.Contains(raw, "integration_outbox") {
...
func analyticsBackfillCursorMACKey() (string, error) {
	...
	if v := strings.TrimSpace(os.Getenv("ROOM_ANALYTICS_BACKFILL_CURSOR_SECRET")); v != "" {
		return v, nil
	}
