...
// Room analytics-backfill topic allowlist (ADR-0039 / AsyncAPI).
var roomAnalyticsBackfillTopics = map[string]string{
	TopicGameplayMetrics: "GameplayMetric",
	TopicMatchCompleted:  "MatchCompleted",
}
...
// AnalyticsBackfillRow is one immutable integration-outbox row for Analytics recovery.
...
// AnalyticsBackfillQuery is a bounded keyset page over integration_outbox_events.
...
// AnalyticsBackfillReader reads owned integration outbox rows without mutation.
...
func (m *MemoryAnalyticsBackfillStore) Append(topic, eventType string, payload json.RawMessage, occurredAt *time.Time) int64 {
	...
	m.rows = append(m.rows, AnalyticsBackfillRow{
		OutboxID: id, Topic: topic, EventType: eventType, Payload: payload, OccurredAt: occurredAt,
	})
	return id
}
...
func (s *Service) AnalyticsBackfill(ctx context.Context, req AnalyticsBackfillRequest) (AnalyticsBackfillResponse, error) {
	...
	wantType := roomAnalyticsBackfillTopics[parsed.topic]
...
func parseAnalyticsBackfillRequest(req AnalyticsBackfillRequest) (parsedAnalyticsBackfill, error) {
	...
	if _, ok := roomAnalyticsBackfillTopics[topic]; !ok {
...
func validateAnalyticsBackfillEnvelope(raw json.RawMessage, topic, wantEventType string) error {
	...
	switch topic {
	case TopicGameplayMetrics:
		if strings.TrimSpace(asString(m["roomId"])) == "" {
			return fmt.Errorf("roomId required")
		}
		vis, _ := m["visibility"].(string)
		if vis != "anonymized_adhoc" && vis != "public_tournament" {
			return fmt.Errorf("visibility")
		}
		if strings.TrimSpace(asString(m["metricType"])) == "" {
			return fmt.Errorf("metricType required")
		}
	case TopicMatchCompleted:
		if strings.TrimSpace(asString(m["roomId"])) == "" {
			return fmt.Errorf("roomId required")
		}
		...
		if _, ok := m["isAbandoned"]; !ok {
			return fmt.Errorf("isAbandoned required")
		}
	}
	return nil
}
