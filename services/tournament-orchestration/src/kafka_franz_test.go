...
func TestDLQHeaders_SanitizedOperationalMetadata(t *testing.T) {
	meta := DLQFailureMeta{
		Consumer:        DefaultTournamentKafkaGroup,
		SourceTopic:     DefaultMatchCompletedTopic,
		SourcePartition: 2,
		SourceOffset:    9,
		AttemptCount:    4,
	...
	want := map[string]string{
		dlqHeaderConsumer:        DefaultTournamentKafkaGroup,
		dlqHeaderSourceTopic:     DefaultMatchCompletedTopic,
