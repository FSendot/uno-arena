...
func (c *franzMatchCompletedClient) Poll(ctx context.Context) ([]ConsumerRecord, error) {
	...
	if errs := fetches.Errors(); len(errs) > 0 {
		// Prefer context errors; otherwise surface the first fetch error.
		for _, fe := range errs {
			if fe.Err != nil {
				if ctx.Err() != nil {
					return nil, ctx.Err()
...
// serviceMatchIngester adapts *Service to MatchCompletedIngester.
type serviceMatchIngester struct {
	svc *Service
}

func (s serviceMatchIngester) IngestMatchCompleted(ctx context.Context, evt MatchCompletedEvent) (map[string]any, error) {
	return s.svc.IngestMatchCompleted(ctx, evt)
}
