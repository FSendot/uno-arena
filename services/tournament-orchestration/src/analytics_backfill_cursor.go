...
const (
	analyticsBackfillCursorPrefix = "tab1."
	// documentedNonProdAnalyticsBackfillCursorMACKey is capability/offline fallback only.
	// Durable API mode requires TOURNAMENT_ANALYTICS_BACKFILL_CURSOR_SECRET before ready/wiring.
	documentedNonProdAnalyticsBackfillCursorMACKey = "unoarena-tournament-analytics-backfill-cursor-v1-nonprod"
)
...
var ErrAnalyticsBackfillCursorSecretRequired = errors.New("TOURNAMENT_ANALYTICS_BACKFILL_CURSOR_SECRET required")
...
func AnalyticsBackfillCursorSecretConfigured() bool {
	if strings.TrimSpace(os.Getenv("TOURNAMENT_ANALYTICS_BACKFILL_CURSOR_SECRET")) != "" {
		return true
	}
...
func analyticsBackfillCursorMACKey() (string, error) {
	...
	if v := strings.TrimSpace(os.Getenv("TOURNAMENT_ANALYTICS_BACKFILL_CURSOR_SECRET")); v != "" {
		return v, nil
	}
