...
const testAnalyticsBackfillCred = "analytics-tournament-credential"

func newAnalyticsBackfillHarness(t *testing.T) (*Server, *MemoryAnalyticsBackfillStore) {
	t.Helper()
	restore := SetAnalyticsBackfillCursorMACKeyForTest("http-tournament-analytics-backfill-cursor")
	t.Cleanup(restore)
	svc, mem := newBackfillSvc(t)
	srv := NewServerWithScopedCreds(svc, testCred, testAnalyticsBackfillCred)
	return srv, mem
}
...
func postAnalyticsBackfill(t *testing.T, mux http.Handler, cred string, body map[string]any) *httptest.ResponseRecorder {
	...
	req := httptest.NewRequest(http.MethodPost, "/internal/v1/tournaments/analytics-backfill", bytes.NewReader(raw))
...
func TestAnalyticsBackfill_Auth(t *testing.T) {
	...
	mux := srv.Routes()
	body := map[string]any{
		"recoveryJobId": "job-auth", "sourceTopic": TopicMatchAssigned, "schemaVersion": 1,
	...
	w = postAnalyticsBackfill(t, mux, testCred, body)
	if w.Code != http.StatusUnauthorized {
...
func TestAnalyticsBackfill_HTTPValidationAndPaging(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		id := mem.Append(TopicMatchAssigned, "TournamentMatchAssigned", assignedEnvelope("http-"+strconv.Itoa(i), ts), &ts)
		if i == 0 {
			firstID = id
		}
		lastID = id
	...
	w := postAnalyticsBackfill(t, mux, testAnalyticsBackfillCred, map[string]any{
		"recoveryJobId": "job-http", "sourceTopic": TopicMatchAssigned, "schemaVersion": 1,
	})
