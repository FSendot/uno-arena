...
const (
	...
	ExpectedSchemaChecksum = "cd3cbfcbd9432cce3dcbc76e904c8e066c01ca0b40ae7889eff9f695e7efaf06"
)
...
// overridden by TOURNAMENT_SCHEMA_CHECKSUM / TOURNAMENT_SCHEMA_VERSION env.
func DefaultSchemaExpectation() SchemaExpectation {
	...
	if v := strings.TrimSpace(os.Getenv("TOURNAMENT_SCHEMA_VERSION")); v != "" {
		exp.MigrationVersion = v
		exp.BootstrapVersion = v
	}
	if c := strings.TrimSpace(os.Getenv("TOURNAMENT_SCHEMA_CHECKSUM")); c != "" {
