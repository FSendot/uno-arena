...
// franzRankingClient wraps a single kgo client for multi-topic consume + per-topic DLQ produce.
type franzRankingClient struct {
	cl  *kgo.Client
	cfg RankingKafkaConfig
}

// franzGameCompletedClient is retained as an alias for older lifecycle wiring/tests.
type franzGameCompletedClient = franzRankingClient

func newFranzRankingClient(cfg RankingKafkaConfig) (*franzRankingClient, error) {
	if len(cfg.Brokers) == 0 {
		return nil, fmt.Errorf("kafka brokers required")
	}
	topics := cfg.normalizedTopics()
	if cfg.Group == "" || len(topics) == 0 {
		return nil, fmt.Errorf("kafka group/topics required")
	}
	for _, t := range topics {
		if cfg.DLQTopicFor(t) == "" {
	...
	return &franzRankingClient{cl: cl, cfg: cfg}, nil
}

func newFranzGameCompletedClient(cfg RankingKafkaConfig) (*franzRankingClient, error) {
	return newFranzRankingClient(cfg)
}

func (c *franzRankingClient) Poll(ctx context.Context) ([]ConsumerRecord, error) {
...
func (c *franzRankingClient) PublishDLQ(ctx context.Context, original ConsumerRecord, meta DLQFailureMeta) error {
	source := firstNonEmpty(meta.SourceTopic, original.Topic)
	dlqTopic := c.cfg.DLQTopicFor(source)
	if dlqTopic == "" {
		return fmt.Errorf("dlq topic not configured for %q", source)
	}
	rec := &kgo.Record{
		Topic:   dlqTopic,
...
func (c *franzRankingClient) Close() error {
	c.cl.Close()
	return nil
}
