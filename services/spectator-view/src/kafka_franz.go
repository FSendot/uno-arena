...
// franzSpectatorSafeClient wraps a single kgo client for consume + DLQ produce.
type franzSpectatorSafeClient struct {
	cl       *kgo.Client
	dlqTopic string
}

func newFranzSpectatorSafeClient(cfg SpectatorSafeKafkaConfig) (*franzSpectatorSafeClient, error) {
	...
	return &franzSpectatorSafeClient{cl: cl, dlqTopic: cfg.DLQTopic}, nil
}

func (c *franzSpectatorSafeClient) Poll(ctx context.Context) ([]ConsumerRecord, error) {
...
func (c *franzSpectatorSafeClient) Commit(ctx context.Context, rec ConsumerRecord) error {
	kr := &kgo.Record{
		Topic:     rec.Topic,
...
func (c *franzSpectatorSafeClient) PublishDLQ(ctx context.Context, original ConsumerRecord, meta DLQFailureMeta) error {
	rec := &kgo.Record{
		Topic:   c.dlqTopic,
...
func (c *franzSpectatorSafeClient) Close() error {
	c.cl.Close()
	return nil
}
