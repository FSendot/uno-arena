...
grep -qEi 'consumers still PENDING|no consumer claim|consumers still' "${LIVE_KAFKA_SCRIPT}" \
  || die "live Kafka script must note consumers remain pending"
...
grep -qF "${ARM64_DIGEST}" "${CONNECT_MANIFEST}" || die "connect manifest missing ARM64 source digest"
grep -qF "${ARM64_DIGEST}" "${JOB_MANIFEST}" || die "register job missing ARM64 source digest"
grep -qF "${ARM64_DIGEST}" "${LOAD_SCRIPT}" || die "load script missing ARM64 source digest"
...
grep -qF 'arm64' "${LOAD_SCRIPT}" || die "load script must verify architecture arm64"
