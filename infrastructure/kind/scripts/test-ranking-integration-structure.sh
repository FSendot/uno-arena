#!/usr/bin/env bash
# Offline structural assertions for test-ranking-integration.sh.
# Ensures ephemeral DB naming, no caller-supplied DB names, quoted identifiers,
# ranking_admin CREATE DATABASE OWNER ranking_runtime, local port 0 PF,
# runtime DSN only, and trap cleanup of only this run's DB + PF child.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ADAPTER="${SCRIPT_DIR}/test-ranking-integration.sh"
...
if ! grep -F 'unoarena_ranking_test_' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must use unoarena_ranking_test_ prefix"
fi
if ! grep -E 'refuse_caller_db_name|RANKING_TEST_DATABASE' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must refuse caller-supplied database name env vars"
fi
if grep -E 'DB_NAME="\$\{RANKING_|DB_NAME="\$\{.*DATABASE' "${ADAPTER}" >/dev/null 2>&1; then
...
if ! grep -F 'unoarena_ranking_test_[a-z0-9]+' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must regex-validate generated DB names as unoarena_ranking_test_[a-z0-9]+"
fi

# CREATE DATABASE via kubectl exec as ranking_admin with OWNER ranking_runtime.
if ! grep -F 'ranking_admin' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must use ranking_admin for CREATE/DROP DATABASE"
fi
if ! grep -E 'CREATE DATABASE' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must CREATE DATABASE for the ephemeral test DB"
fi
if ! grep -E 'OWNER[[:space:]]+\$\{RUNTIME_USER\}|OWNER ranking_runtime' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must CREATE DATABASE ... OWNER ranking_runtime"
fi
...
if ! grep -E 'svc/\$\{SERVICE_NAME\}|svc/postgres-ranking' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must port-forward svc/postgres-ranking"
fi
...
# Runtime user DSN to temp DB only; drives make test-ranking-integration.
if ! grep -F 'ranking_runtime' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must use ranking_runtime for the test DSN"
fi
if ! grep -F 'RANKING_POSTGRES_URL' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must export RANKING_POSTGRES_URL"
fi
if ! grep -E '127\.0\.0\.1:\$\{LOCAL_PORT\}/\$\{DB_NAME\}' "${ADAPTER}" >/dev/null 2>&1; then
  die "RANKING_POSTGRES_URL must target allocated LOCAL_PORT and generated DB_NAME"
fi
if grep -E 'RANKING_POSTGRES_URL="\$\{RANKING_POSTGRES_URL:-' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must not honor a pre-set RANKING_POSTGRES_URL over its ephemeral DB"
fi
if ! grep -E 'make[[:space:]]+test-ranking-integration' "${ADAPTER}" >/dev/null 2>&1; then
  die "adapter must run make test-ranking-integration"
fi
...
runtime_dir="$(mktemp -d "${TMPDIR:-/tmp}/ranking-int-struct.XXXXXX")"
...
SAFE_DB_PREFIX="unoarena_ranking_test_"
EOF
...
assert_name_rejected "ranking"
assert_name_rejected "postgres"
assert_name_rejected "template0"
assert_name_rejected "template1"
assert_name_rejected ""
assert_name_rejected "unoarena_ranking_test_"
assert_name_rejected "unoarena_ranking_test_ABC"
assert_name_rejected "unoarena_ranking_test_ab-cd"
assert_name_rejected "evil;drop"
assert_name_accepted "unoarena_ranking_test_abc123"
assert_name_accepted "unoarena_ranking_test_deadbeefcafebabe"

quoted="$(bash -c "source '${runtime_dir}/helpers.sh'; sql_quote_ident 'unoarena_ranking_test_abc123'")"
[[ "${quoted}" == '"unoarena_ranking_test_abc123"' ]] || die "sql_quote_ident produced ${quoted}"

# refuse_caller_db_name must die when env is set.
if bash -c "source '${runtime_dir}/helpers.sh'; RANKING_TEST_DATABASE=nope refuse_caller_db_name" >/dev/null 2>&1; then
  die "refuse_caller_db_name must reject RANKING_TEST_DATABASE"
fi

echo "ok kind-test-ranking-integration-structure"
