#!/usr/bin/env bash
# Isolated kubectl port-forward to Ranking ClusterIP using local port 0.
# Only cleans up this script's child job — never kills by port or reused PID.
# Prints RANKING_BASE_URL=http://127.0.0.1:<port> for callers.
set -euo pipefail
set -m
...
REMOTE_PORT=8080
SERVICE_NAME="${RANKING_SERVICE_NAME:-ranking}"
PF_PID=""
PF_LOG=""
LOCAL_PORT=""
...
PF_LOG="$(mktemp "${TMPDIR:-/tmp}/ranking-pf.XXXXXX")"
...
for _ in $(seq 1 40); do
  if ! kill -0 "${PF_PID}" 2>/dev/null; then
    die "ranking port-forward exited early (see ${PF_LOG})"
  fi
  if jobs -pr 2>/dev/null | grep -qx "${PF_PID}"; then
    if parsed="$(grep -E "Forwarding from 127\\.0\\.0\\.1:[0-9]+ -> ${REMOTE_PORT}" "${PF_LOG}" 2>/dev/null | head -n1 || true)" \
      ...
      if [[ -n "${LOCAL_PORT}" ]] && [[ "${LOCAL_PORT}" =~ ^[0-9]+$ ]] \
        ...
        if nc -z 127.0.0.1 "${LOCAL_PORT}" 2>/dev/null; then
          if ! kill -0 "${PF_PID}" 2>/dev/null; then
            die "ranking port-forward died after port became reachable (see ${PF_LOG})"
          fi
          ready=1
          break
        fi
...
[[ "${ready}" == "1" ]] || die "ranking port-forward to 127.0.0.1 failed (see ${PF_LOG})"
[[ -n "${LOCAL_PORT}" ]] || die "failed to parse Forwarding from line in ${PF_LOG}"

export RANKING_BASE_URL="http://127.0.0.1:${LOCAL_PORT}"
echo "RANKING_BASE_URL=${RANKING_BASE_URL}"
