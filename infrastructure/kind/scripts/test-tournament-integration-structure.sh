...
if [[ "$(grep -Fc -- '-parallel 2 -timeout 480s' <<<"${recipe}")" -ne 2 ]]; then
  die "both Tournament integration lanes must bound local parallelism and allow the reviewed eight-minute budget"
fi
