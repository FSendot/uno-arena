#!/usr/bin/env bash
# Explicit, networked Identity Postgres store integration against kind.
# Creates an ephemeral DB named unoarena_identity_test_<random>, port-forwards
# postgres-identity (local port 0), runs make test-identity-integration as
# identity_runtime against ONLY that DB, then terminates backends + DROP DATABASE.
...
REMOTE_PORT=5432
SERVICE_NAME="postgres-identity"
ADMIN_USER="identity_admin"
RUNTIME_USER="identity_runtime"
...
refuse_caller_db_name() {
  if [[ -n "${IDENTITY_TEST_DATABASE:-}" ]]; then
    die "refusing caller-supplied IDENTITY_TEST_DATABASE; harness generates its own name"
  fi
  if [[ -n "${IDENTITY_INTEGRATION_DATABASE:-}" ]]; then
    die "refusing caller-supplied IDENTITY_INTEGRATION_DATABASE; harness generates its own name"
  fi
  if [[ -n "${IDENTITY_POSTGRES_DATABASE:-}" ]]; then
    die "refusing caller-supplied IDENTITY_POSTGRES_DATABASE; harness generates its own name"
  fi
}
...
validate_generated_db_name() {
  local name="$1"
  if [[ ! "${name}" =~ ^unoarena_identity_test_[a-z0-9]+$ ]]; then
    die "refusing unsafe generated database identifier: ${name}"
  fi
  case "${name}" in
    identity|postgres|template0|template1|"")
...
psql_admin() {
  # stdin SQL or -c via args; always identity_admin against postgres maintenance DB.
...
create_temp_database() {
  ...
  # OWNER identity_runtime so the DML role can apply test schema DDL in an empty DB.
...
RUNTIME_PASS="$(kubectl -n "${KIND_NAMESPACE}" get secret uno-arena-local-credentials \
  -o jsonpath='{.data.identity-runtime-password}' | base64 -d)"
[[ -n "${RUNTIME_PASS}" ]] || die "missing identity-runtime-password in uno-arena-local-credentials"

PF_LOG="$(mktemp "${TMPDIR:-/tmp}/identity-pg-pf.XXXXXX")"
...
for _ in $(seq 1 40); do
  if ! kill -0 "${PF_PID}" 2>/dev/null; then
    die "postgres-identity port-forward exited early (see ${PF_LOG})"
  fi
  if jobs -pr 2>/dev/null | grep -qx "${PF_PID}"; then
    if parsed="$(grep -E "Forwarding from 127\\.0\\.0\\.1:[0-9]+ -> ${REMOTE_PORT}" "${PF_LOG}" 2>/dev/null | head -n1 || true)" \
      ...
      if [[ -n "${LOCAL_PORT}" ]] && [[ "${LOCAL_PORT}" =~ ^[0-9]+$ ]] \
        ...
        if nc -z 127.0.0.1 "${LOCAL_PORT}" 2>/dev/null; then
          if ! kill -0 "${PF_PID}" 2>/dev/null; then
            die "postgres-identity port-forward died after port became reachable (see ${PF_LOG})"
          fi
          ready=1
          break
        fi
...
[[ "${ready}" == "1" ]] || die "postgres-identity port-forward to 127.0.0.1 failed (see ${PF_LOG})"
...
export IDENTITY_POSTGRES_URL="postgres://${RUNTIME_USER}:${RUNTIME_PASS}@127.0.0.1:${LOCAL_PORT}/${DB_NAME}?sslmode=disable"

echo "running Identity store integration against database ${DB_NAME} on 127.0.0.1:${LOCAL_PORT}"
cd "${REPO_ROOT}"
make test-identity-integration
echo "ok kind-test-identity-integration"
