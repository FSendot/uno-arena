#!/usr/bin/env bash
# Explicit, networked Ranking Postgres store integration against kind.
# Creates an ephemeral DB named unoarena_ranking_test_<random>, port-forwards
# postgres-ranking (local port 0), runs make test-ranking-integration as
# ranking_runtime against ONLY that DB, then terminates backends + DROP DATABASE.
...
REMOTE_PORT=5432
SERVICE_NAME="postgres-ranking"
ADMIN_USER="ranking_admin"
RUNTIME_USER="ranking_runtime"
...
refuse_caller_db_name() {
  if [[ -n "${RANKING_TEST_DATABASE:-}" ]]; then
    die "refusing caller-supplied RANKING_TEST_DATABASE; harness generates its own name"
  fi
  if [[ -n "${RANKING_INTEGRATION_DATABASE:-}" ]]; then
    die "refusing caller-supplied RANKING_INTEGRATION_DATABASE; harness generates its own name"
  fi
  if [[ -n "${RANKING_POSTGRES_DATABASE:-}" ]]; then
    die "refusing caller-supplied RANKING_POSTGRES_DATABASE; harness generates its own name"
  fi
}
...
validate_generated_db_name() {
  local name="$1"
  if [[ ! "${name}" =~ ^unoarena_ranking_test_[a-z0-9]+$ ]]; then
    die "refusing unsafe generated database identifier: ${name}"
  fi
  case "${name}" in
    ranking|postgres|template0|template1|"")
...
psql_admin() {
  # stdin SQL or -c via args; always ranking_admin against postgres maintenance DB.
  kubectl -n "${KIND_NAMESPACE}" exec -i deploy/postgres-ranking -- \
...
create_temp_database() {
  ...
  # OWNER ranking_runtime so the DML role can apply test schema DDL in an empty DB.
...
RUNTIME_PASS="$(kubectl -n "${KIND_NAMESPACE}" get secret uno-arena-local-credentials \
  -o jsonpath='{.data.ranking-runtime-password}' | base64 -d)"
[[ -n "${RUNTIME_PASS}" ]] || die "missing ranking-runtime-password in uno-arena-local-credentials"

PF_LOG="$(mktemp "${TMPDIR:-/tmp}/ranking-pg-pf.XXXXXX")"
...
for _ in $(seq 1 40); do
  if ! kill -0 "${PF_PID}" 2>/dev/null; then
    die "postgres-ranking port-forward exited early (see ${PF_LOG})"
  fi
  if jobs -pr 2>/dev/null | grep -qx "${PF_PID}"; then
    if parsed="$(grep -E "Forwarding from 127\\.0\\.0\\.1:[0-9]+ -> ${REMOTE_PORT}" "${PF_LOG}" 2>/dev/null | head -n1 || true)" \
      ...
      if [[ -n "${LOCAL_PORT}" ]] && [[ "${LOCAL_PORT}" =~ ^[0-9]+$ ]] \
        ...
        if nc -z 127.0.0.1 "${LOCAL_PORT}" 2>/dev/null; then
          if ! kill -0 "${PF_PID}" 2>/dev/null; then
            die "postgres-ranking port-forward died after port became reachable (see ${PF_LOG})"
          fi
          ready=1
          break
        fi
...
[[ "${ready}" == "1" ]] || die "postgres-ranking port-forward to 127.0.0.1 failed (see ${PF_LOG})"
...
export RANKING_POSTGRES_URL="postgres://${RUNTIME_USER}:${RUNTIME_PASS}@127.0.0.1:${LOCAL_PORT}/${DB_NAME}?sslmode=disable"

echo "running Ranking store integration against database ${DB_NAME} on 127.0.0.1:${LOCAL_PORT}"
cd "${REPO_ROOT}"
make test-ranking-integration
echo "ok kind-test-ranking-integration"
