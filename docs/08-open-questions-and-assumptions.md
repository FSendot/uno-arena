...
- Current capability-mode adapters (`GATEWAY_CAPABILITY_MODE` / `ROOM_CAPABILITY_MODE` / `IDENTITY_CAPABILITY_MODE` / `ANALYTICS_CAPABILITY_MODE` / `SPECTATOR_CAPABILITY_MODE`, GI explicit memory, HTTP bridge transforms) remain distinct from the durable adapters. Identity/Room/Ranking/Tournament Postgres, Game Integrity KurrentDB, Analytics ClickHouse, Spectator/Ranking/Tournament Redis projections, Room Redis timers, and Gateway Redis admission/LiveFeed adapters are implemented when their context-owned connection settings are present. Declared franz-go consumers are implemented for Gateway session invalidation, Tournament match completion, Ranking multi-topic ingestion, Spectator spectator-safe events, and the nine Analytics topics. Debezium Connect owns the append-only Postgres outbox routes and Debezium Server owns Room realtime-to-Redis delivery; applications do not poll those outboxes. Context-owned Room/Tournament/Ranking Analytics backfills, the Room spectator recovery snapshot, and Spectator/Analytics rebuild workers are implemented; the two projection-rebuilder Deployments remain disabled in every overlay until their full Kafka→HTTP→Redis/ClickHouse recovery lanes pass live in kind. Existing live store and CDC checks are explicit targets and do not by themselves claim a final clean-cluster deployment proof.
