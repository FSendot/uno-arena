...
## Decision
Use Helm releases applied from the pipeline. Application contexts use their context-owned charts, while the portable observability platform uses the repository-owned `infrastructure/observability/helm/uno-arena-observability` chart. Deployment jobs run `helm upgrade --install` against the cluster using a kubeconfig injected as a masked GitLab CI variable or the local runner host's active Kubernetes context. Readiness gates block each dependent stage until its resources are healthy. Local-only MinIO storage and bucket provisioning remain part of the disposable kind foundation rather than the portable chart; production values reference externally provisioned S3 and Secrets without provisioning cloud resources. Rollback is `helm rollback <release> <revision>`. Production deployment is intentionally documented as a promotion model for this checkpoint, not implemented as executable CI.
