...
## Decision
Use Helm releases applied from the pipeline. Each service's `deploy-staging` and `deploy-production` jobs run `helm upgrade --install` against the cluster using a kubeconfig injected as a masked GitLab CI variable. The readiness gate is `kubectl rollout status`, which blocks the next stage until pods are healthy. Rollback is `helm rollback <release> <revision>`.
