...
- Room Gameplay uses one dedicated lifecycle-bound state-machine pod as the exclusive executor for each active room, with native Postgres partitioning and indexes by `roomId` retained inside its authoritative context-owned database. Replacement pods reconstruct their assigned aggregate from Postgres.
- Dedicated room pods connect through Room-owned PgBouncer transaction pooling with lazy `minConns=0` / `maxConns=1` client pools and short idle lifetime. Router/controller pools are separately bounded so 100,000 pods do not imply 100,000 ordinary service-sized Postgres pools.
- The production starting envelope uses six PgBouncer replicas with 50,000 admitted clients each and 32 default plus 8 reserve backend connections per replica for the Room database/user pair. This 300,000-client/240-backend envelope must be reconciled with measured transaction concurrency, PgBouncer CPU/file-descriptor limits, Postgres `max_connections`, failover headroom, and uneven Kubernetes Service connection distribution.
...
- Room Gameplay's runtime controller turns those durable room lifecycles into dedicated pods in bounded admission batches; a shared router watches ready pod assignments instead of creating 100,000 per-room Services.
- The controller creates one bare Pod rather than a Deployment/StatefulSet per room, avoiding an additional workload controller object per room. Pod names are bounded hashes plus generation; Postgres remains the lifecycle source rather than duplicating assignments into CRDs.
- Multiple controller replicas claim bounded Postgres assignment batches with `SKIP LOCKED` leases. Per-replica concurrency and Kubernetes client rate limits shape API-server pressure; deterministic pod names make retry after controller crash idempotent without a single elected leader.
- The production admission starting envelope is 32 controller replicas, each claiming at most 64 assignments and executing at most 16 Kubernetes operations concurrently on the one-second reconcile cadence. Its theoretical claim budget exceeds 100,000 rooms per minute, but acceptance depends on API-server, scheduler, node provisioning, image distribution, and readiness measurements; claim arithmetic alone is not a throughput guarantee.
- Kind proves the same router/controller/PgBouncer/dedicated-pod behavior with small room batches; it does not claim a 100,000-pod benchmark. Non-Kubernetes capability mode validates semantics only.
- Pod scheduling is asynchronous to `CreateRoom`. Admission backlog is visible as `room_starting`; the first tournament runtime readiness is joined through the Room-owned `RoomRuntimeReady` outbox fact before assignment visibility, and clients use bounded backoff for casual startup or later runtime replacement rather than holding creation transactions open.
- CLI startup/replacement reads honor `Retry-After` with jitter, cap each interval at five seconds, and use a configurable 60-second default total budget. Mutations with unknown outcomes are never retried automatically.
...
- Timer rebuilds use one context-wide lease, and Game Integrity repair uses separately bounded `room-integrity-reconciler` claims. Neither global workload scales with the number of dedicated Room pods or router replicas.
