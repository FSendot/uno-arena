...
- **Capability status:** Capability mode uses real HTTP service paths with bounded memory limiters/session repo and explicit GI/Analytics memory; it must not be read as the durable path. Identity/Room/Ranking/Tournament Postgres, GI KurrentDB, Analytics ClickHouse HTTP, Spectator/Ranking/Tournament Redis projections, Room Redis timers, and Gateway Redis rate-limit + LiveFeed SSE adapters are implemented when their context-owned DSNs/URLs are set. Kind Debezium Kafka Connect (four outbox connectors) and Debezium Server (Room realtime → Redis) infrastructure are implemented. Declared durable franz-go consumers are implemented when `KAFKA_BROKERS` is set: Gateway `identity.session.invalidated`; Tournament `room.match.completed`; Ranking `room.game.completed` / `tournament.players.advanced` / `tournament.completed`; Spectator `room.spectator-safe.events`; Analytics nine configured topics. Ranking leaderboard and Tournament bracket Redis adapters have real kind Redis integration coverage. Live CDC and rebuild-worker delivery proofs remain explicit targets; **final clean-cluster live proof has not yet run**.
...
| Spectator/Analytics post-retention recovery uses Kafka rebuild-request topics plus context-owned internal snapshot/backfill APIs; APIs and rebuilder workers are implemented, Deployments stay disabled pending live recovery tests | `02-bounded-context-architecture.md`, `03-communication-patterns.md`, `04-persistence-by-context.md`, `contracts/asyncapi/kafka-v1.yaml`, `docs/adr/0039-context-owned-snapshot-backfill-and-kafka-rebuild-requests.md` |
...
| Istio Ambient provides mandatory production east-west L4 mTLS and SPIFFE workload identity without per-pod sidecars; L7 waypoints are opt-in | `01-context-and-container-view.md`, `03-communication-patterns.md`, `06-cross-cutting-concerns.md`, `docs/adr/0025-istio-ambient-for-production-mtls.md` |
...
| Helm Service/peer port 8080; Room/Tournament workers gated by env; Spectator/Analytics projection rebuilders implemented but disabled pending live recovery tests; `/ready` not weakened for missing durable adapters | `README.md`, `services/*/helm/**`, `06-cross-cutting-concerns.md` |
