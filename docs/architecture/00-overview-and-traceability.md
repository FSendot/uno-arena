...
The package should eventually include polished architecture visuals, but the documents below already stand on their own.

Prompt brief for a cover asset:

> Clean editorial system-architecture illustration for a multiplayer card-game platform. Show one external client boundary feeding a BFF, then split into room gameplay, integrity, tournament, ranking, identity, spectator, and analytics services. Use restrained color, crisp geometry, subtle depth, and labels that emphasize bounded-context separation and event-driven flow.
