...
- **Capability status:** architecture still requires Kafka and remaining CDC delivery per context. Capability mode uses real HTTP service paths with bounded memory limiters/session repo and explicit GI/Analytics memory; it must not be read as claiming Kafka/Debezium adapters are wired. Identity/Room/Ranking/Tournament Postgres, GI KurrentDB, Analytics ClickHouse HTTP, Spectator Redis projection, and Gateway Redis rate-limit + LiveFeed SSE durable adapters are implemented when their writer DSNs/URLs are set. Gateway Kafka SessionInvalidated consumer and Debezium player-feed sink remain pending. Analytics Kafka ingestion remains pending.
...
- A consumer that exceeds Kafka retention enters an explicit sanitized snapshot or context-owned backfill path; it never advances checkpoints across an unexplained gap.
