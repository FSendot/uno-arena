...
- The CLI retries only safe room/assignment reads that return `503 room_starting`, honors `Retry-After` with jitter and a five-second maximum interval, and stops after the configurable `UNOARENA_ROOM_START_TIMEOUT_SECONDS` budget (60 seconds by default). It never retries a mutation after an unknown outcome.
