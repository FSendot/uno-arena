...
- A `stale_sequence` command outcome is returned as HTTP `409 Conflict` with the
  structured rejected command result intact; other domain rejections remain HTTP `200`.
...
- At the Room boundary, classify and authenticate the original route before runtime lookup. Only the stable Room router may replace that scoped authority with its router credential and pinned generation for the internal pod hop.
...
- Kafka-bound connectors place `traceparent` and `tracestate` into standard Kafka headers. Room's Debezium Server places them into realtime Redis entry metadata; direct Kafka producers inject and consumers extract the same headers. W3C baggage is never persisted through an outbox.
...
- The CLI retries only explicitly safe GETs: authenticated identity setup plus room snapshots and tournament assignments that encounter a transient `502`, `504`, or timeout, and Room-owned reads that return `503 room_starting`. It honors `Retry-After` with jitter and a five-second maximum interval, and stops after the configurable `UNOARENA_ROOM_START_TIMEOUT_SECONDS` budget (60 seconds by default). It never retries a mutation after an unknown outcome. A headless bot may reconcile a mutation transport `502`, `503`, `504`, or timeout by observing fresh authoritative snapshots for the same configured safe-read budget and never beyond its overall deadline; this includes a delayed dedicated-runtime replacement between best-of-three games. It continues only when the Room is terminal or its sequence advanced beyond the submitted sequence through that command, a peer, or a Room-owned timer. It never replays the failed mutation envelope against unchanged state and leaves unrecovered or non-terminal runs as failures.
...
| BFF | Room Gameplay router → dedicated Room pod | Authorization-first synchronous `roomId` routing + pessimistic Room row lock | Preserve one stable bounded-context endpoint while every active room has one exclusive state-machine executor | Stable Room validates the Gateway credential and BFF-derived principal before pod lookup; router forwards only to the ready pod assigned to `roomId`; idempotent by `commandId`; stale by `expectedSequenceNumber`; bounded `SELECT ... FOR UPDATE`; rejected commands emit audit records only and never append Game Integrity; no blind retries after unknown mutation |
...
| Room Timer Worker | Room Postgres → Redis timer index | Context-wide timer-index rebuild under a Room-owned lease | Recover the non-authoritative timer index without multiplying rebuilds by runtime or timer replicas | One lease holder rebuilds; partitioned ordinary timer claims continue horizontally; runtimes and router never rebuild |
...
| Room Integrity Reconciler | Room Postgres → Game Integrity | Bounded repair claims with `FOR UPDATE SKIP LOCKED` | Repair append-confirmed/local-commit-failed markers independently of active runtime placement | Explicit replica/batch limits; one claimant per marker; no full-table scan per runtime |
...
| Room Gameplay | Tournament Orchestration | `room.runtime.ready` | Open player-visible tournament assignment only after the first durable Ready observation | Tournament validates `(tournamentId, roundNumber, slotId, roomId)`, dedupes by `eventId`, and keeps later replacement generations visible |
...
| Room Gameplay -> Game Integrity append | No blind retry after an unknown write result; Game Integrity resolves an ambiguous first append only by authenticating and comparing the committed deterministic event | none for simple append failure; command fails before broadcast | Exact committed event confirms success, a competing payload returns `conflicting_duplicate`, or unprovable state fails closed |
