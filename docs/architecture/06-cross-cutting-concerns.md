...
- AsyncAPI under `contracts/asyncapi/kafka-v1.yaml` formalizes topic names, event schemas (including Room `GameCompleted`/`MatchCompleted` bodies), producers, consumers, partition keys, idempotency keys, and schema-version policy. The Kafka envelope is authoritative; offline HTTP bridges are documented destination-specific transforms of the same event names and canonical fields. Durable Kafka adapters are implemented in consuming services when `KAFKA_BROKERS` is configured; kind live end-to-end CDC proofs remain explicit optional targets.
...
- Helm Kubernetes Services and peer URLs use port 8080 consistently. Room timer and Tournament provisioning/seeding workers stay `enabled: false` outside kind until ops enable them after durable loops exist. Spectator/Analytics projection-rebuilder workers implement ADR-0039 rebuild-request consumers plus context-owned snapshot/backfill callers and remain `projectionRebuilder.enabled=false` in default/staging/production/`kind` until real end-to-end worker live recovery tests pass (structure checks alone are not enablement).
- Do not weaken `/ready` outside explicit capability mode: configured Gateway requires Redis rate-limit + LiveFeed clients (ping every configured Redis client); Room Postgres session and Game Integrity KurrentDB adapters intentionally keep the service not-ready until durable stores are reachable. Gateway Kafka SessionInvalidated consumption is implemented when `KAFKA_BROKERS` is set; Debezium player-feed decode is implemented on Gateway LiveFeed, while live Postgres→Redis delivery proofs remain explicit optional targets. **Final clean-cluster live proof has not yet run.**
